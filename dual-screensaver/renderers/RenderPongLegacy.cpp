...
void RenderPongLegacy(HDC memDC, ScreenData* data, int width, int height, const RECT& rect) {
    auto& state = data->GetCustomState<PongLegacyState>(32);
...
// Register as Screensaver ID 32
REGISTER_SCREENSAVER(
    32,
    L"Pong (Legacy)",
    "pong-legacy",
    { "pong-legacy", "legacy-pong", "pong1972", "pong-old" },
    WRAP_LEGACY(PongLegacy::RenderPongLegacy),
    GetPongSettings()
);
